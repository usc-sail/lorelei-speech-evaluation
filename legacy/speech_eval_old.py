...
# create a list with all confidence scores -------------------------------------
def collect_confidence(frames):
	confidence = []
	for frame in frames:
		confidence.append(frame['TypeConfidence'])
	return confidence


# keep only frames above a confidence level ------------------------------------
def filter_by_confidence(frames,confidence,threshold):
	out_frames = []
...
def single_point(params):
	...
	out_frames = filter_by_confidence(output_frames,confidence,threshold)
...
#		print (args)
		...
		pr_resolution = 500 # points used to approximate the PR curve
	else:
